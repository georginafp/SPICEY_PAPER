...
# 1. Identifying anchors between scRNA-seq and scATAC-seq datasets ----
...
# 2. Annotate scATAC-seq cells via label transfer -----
...
p1 <- DimPlot(pbmc.atac, group.by = "predicted.id", label = TRUE) + NoLegend() +
...
p1 <- ggplot(predictions, aes(Var1, Var2, fill = Freq)) +
  ...
  xlab("Cell type annotation (RNA)") + ylab("Predicted cell type label (ATAC)") +
...
p1 + p2
saveRDS(pbmc.atac, "/homes/users/gfuentes/scratch/projects/spicey_paper/PBMC/data/final_pbmc_atac.rds")
